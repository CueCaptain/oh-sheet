...
function CueDraggableHeaderPiece(props: {
...
export function CueListSortWizard() {
  ...
  useEffect(() => {
    if (server.sortedHeaders.length === 0) {
      server.setSortedHeaders(
        Object.keys(server.data.cues[server.data.currentPtr]),
      );
    }
  }, [server]);
  ...
  const addHeaderOptions: (JSX.Element | null)[] = Object.keys(
    server.data.cues[server.data.currentPtr],
  )
    .filter(val => !server.sortedHeaders.includes(val))
    .map(key => {
      if (
        key === 'cue' ||
        key === 'item' ||
      ...
      return (
        <Box m={'xs'} key={key}>
          <CueDraggableHeaderPiece
            showDragIcon={false}
            item={key}
            onClick={addHeader(key)}
            icon={'add'}
          />
        </Box>
      );
  ...
  const deleteHeaderOptions: (JSX.Element | null)[] = Object.keys(
    server.data.cues[server.data.currentPtr],
  )
    .filter(val => server.sortedHeaders.includes(val))
    .map(key => {
  ...
  const renderView = () => {
    if (isAddView)
      return (
        <Box>
          <Flex w={'100%'}>
            <Button w={'100%'} m={'xs'} onClick={toggleAddView}>
              {'Back'}
            </Button>
          </Flex>
          {addHeaderOptions.length > 0 ? (
            addHeaderOptions
          ) : (
    ...
    if (isDeleteView)
      return (
        <Box>
          <Flex w={'100%'}>
            <Button w={'100%'} m={'xs'} onClick={toggleDeleteView}>
              {'Back'}
            </Button>
          </Flex>
          {deleteHeaderOptions.length > 0 ? (
            deleteHeaderOptions
          ) : (
            <Flex w={'100%'} justify={'center'} align={'center'}>
              No headers to delete
            </Flex>
          )}
        </Box>
      );
    ...
    <Box h={'100%'} w={'100%'}>
      ...
      {isAddView || isDeleteView ? (
        ...
        <ReactGridLayout
          cols={1}
          rowHeight={60}
          layout={server.sortedHeaders.map((v, i) => {
            return {w: 1, h: 1, x: 0, y: i, i: v};
          })}
