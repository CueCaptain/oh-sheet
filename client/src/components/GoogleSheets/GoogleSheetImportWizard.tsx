...
export default function GoogleSheetImportWizard(
  props: IGoogleSheetImportWizardProps,
) {
  ...
  const getSheetNames = () => {
    setLoading(true);
    const url = new URL(
      `http://${window.location.hostname}:${import.meta.env.VITE_OHSHEET_BACKEND_SERVER_PORT}/cuesheet/get_sheet_names_from_google_sheets`,
    );
  ...
  const loadGoogleSheet = () => async () => {
    ...
    fetch(
      `http://${window.location.hostname}:${import.meta.env.VITE_OHSHEET_BACKEND_SERVER_PORT}/cuesheet/google_sheet`,
      {
        method: 'POST',
        headers: {
    ...
    <Box>
      {sheets.length === 0 ? (
        <Flex direction={'column'}>
          <Flex mb={'md'}>
            <Input
              id="GoogleSheetInput-no-key-down"
              style={{flexGrow: 1}}
              placeholder="Google Sheet URL"
              onChange={e => setNewSheetUrl(e.target.value)}
            />
          </Flex>
          <Button mb={'xs'} onClick={getSheetNames}>
            Import
          </Button>
          <Button
            variant={'outline'}
            onClick={() =>
              window.open(
                'https://docs.google.com/spreadsheets/d/1CccKfkQ8Y-mryWyUK21l6oxeJV3Wyhb7OmEo1qv18kk/template/preview',
                '_blank',
              )
            }
          >
