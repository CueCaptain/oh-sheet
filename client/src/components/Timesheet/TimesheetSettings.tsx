...
export default function TimesheetSettings () {
    ...
    return (
        <Box>
            <Modal id='idk' opened={opened} onClose={close} title="Timesheet Settings" centered>
                <Box w={'100%'} h={'100%'}>
                    {server.data.cues.length > 0 ? <CueListSortWizard /> : "A timesheet must be loaded before continuing with settings"}
                </Box>
            </Modal>
