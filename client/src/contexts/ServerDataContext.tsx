...
interface ServerDataInterface {
  ...
  setMessageData: (
    key: 'operatorMessage' | 'stageTimerMessage',
    message: string,
  ) => void;
}
...
export default function ServerDataProvider({
  ...
  useEffect(() => {
    const _socket = io(
      `http://${window.location.hostname}:${import.meta.env.VITE_OHSHEET_BACKEND_SERVER_PORT}`,
  ...
  const incrementCurrentPtr = () => {
    const newPtr =
      serverData.currentPtr >= serverData.cues.length - 1
        ? 0
        : ++serverData.currentPtr;
