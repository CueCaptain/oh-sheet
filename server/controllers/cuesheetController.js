...
export const getSheetNamesFromGoogleSheets = async (req, res) => {
    ...
    fetch(spreadsheetDataUrl).then(response => {
        ...
        if(spreadsheetData?.sheets?.length > 0){
            ...
            res.status(400).send({message: "Something is wrong with your Spreadsheet Url"});
        }
    });
}
...
export const postGoogleSheet = async (req, res) => {
    ...
    const spreadsheetId = spreadsheetUrl.match(/[-\w]{25,}/);
    if(!spreadsheetUrl) res.status(400).send({message: "Missing spreadsheetUrl"});
    else if(!sheetName) res.status(400).send({message: "Missing sheetName"});
    else if(!spreadsheetId) res.status(400).send({message: "Could not parse spreadsheetUrl"});
    else {
        ...
        fetch(sheetUrl).then(response => {
            ...
            fetch(spreadsheetDataUrl).then(response => {
                ...
                for(let i = 0; i < cues.length; i++){
                    const cue = cues[i];
                    if(cueMap[cue.cue]) {
                        res.status(400).send({message: "Cue Numbers need to be unique."});
                        return false; 
                    }
                    cueMap[cue.cue] = true;
                    ...
                    if(!start.isValid() || !end.isValid()){
                        res.status(400).send({message: "Double check the values in your start or end columns. The time must be in `hh:mm:ss a` format"});
                        return false;
                    }
...
const validateHeaders = (headers) => {
    ...
    if(!found) {
        return {success: false, message: "Headers are missing. Please ensure the table headers are on the row entered and all required headers are included (cue, start, end, item)"};
    }
